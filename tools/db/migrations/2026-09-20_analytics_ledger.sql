...
--   即「次数 / token」两样全算不出。本迁移只补**存储**；采集修复在代码侧。
--
-- **口径（用户 2026-09-19 裁决，勿再引入价格/成本）**：本期**只记 token 数**——输入多少、输出多少。
-- 以后按 token 计价，钱由 token 换算，平台不必自己算。故建表**不带** `cost` 与价格快照列。
--
-- 幂等：建表用 CREATE TABLE IF NOT EXISTS；删列先查 information_schema 再 PREPARE
-- （沿用 2026-09-16_chinese_interpretation_columns.sql 的固定套路）。2026-09-15 那次迁移
-- 因级联删题静默清空 50 行的教训见 docs/superpowers/plans/2026-09-15-chinese-passages-standalone.md Task 9。
