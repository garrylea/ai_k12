...
--
-- 做什么：
--   1) idx_sa_parent_created (parent_id, created_at) —— 家长端列表 + 顶栏 Banner
--   2) idx_sa_created_at     (created_at)            —— 本次新增的保留期清理
--
-- 为什么：
--   * 列表查询 `WHERE parent_id=? [AND is_read=0] ORDER BY created_at DESC, id DESC`
--     此前只有 (parent_id, is_read) 可用 → **filesort**。加了 (parent_id, created_at)
--     后范围扫描出来的行天然按 created_at 有序，排序可省。
--   * 本次新增的 countOlderThan / deleteOlderThan 都是 `WHERE created_at < ?`，
--     无索引时是全表扫。单列 created_at 索引让它是区间扫描。
...
-- ── 1. 家长端列表 + Banner：(parent_id, created_at) ──
SET @idx_exists := (
