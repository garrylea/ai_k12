...
class TestPipelineConfig:
    def test_default_paths_point_to_crawler_data_and_output(self):
    ...
    def test_explicit_args_override_defaults(self, tmp_path):
        ...
        assert config.data_dir == tmp_path / "in"
        assert config.output_dir == tmp_path / "out"

    def test_env_vars_override_defaults(self, monkeypatch, tmp_path):
        monkeypatch.setenv("CRAWLER_DATA_DIR", str(tmp_path / "env_data"))
        monkeypatch.setenv("CRAWLER_OUTPUT_DIR", str(tmp_path / "env_output"))
        ...
        config = PipelineConfig.from_env()
        assert config.data_dir == tmp_path / "env_data"
    ...
    def test_openai_api_key_falls_back_to_anthropic(self, monkeypatch):
