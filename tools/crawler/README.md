...
**同名不同来源会自动改名并告警。** 区县取自索引页表头时，整张汇总表会共用同一个区县（如 `北京各`），
不同详情页可能解析出完全相同的文件名。写盘前会比对 `meta.json` 里的 `source_url`：同名且同来源照常覆盖
（重跑保持幂等），同名但来源不同则在扩展名前追加来源后缀（详情页 URL 末段数字，无数字段则用来源 md5 前 6 位），
另存为 `…-试卷-76320.pdf` 并打印 `警告：文件名冲突，改名保存 …`。改名是确定性的，`--force` 重跑不会堆积文件。
**看到这条警告说明两个文件都完整保留了**——过去这里会静默覆盖，命令报 `Downloaded: 71` 而磁盘只剩 3 个文件。
