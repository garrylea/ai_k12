...
所以定位**只按每句原文**（`_norm`：去空白 + 引号归一后全等），**不看行号、不看数组下标、不做长度断言**：
人工删行 / 漏填 / 调序、或者管线把诗重新切过句，都不会串句。
（引号归一挡的是排版差异，不是内容差异：库里 `“…”` vs 文档 `"…"` 归一后相等，
同一句才认得出；真改了字的仍然定位不到、照旧进清单。）
...
#: 引号归一：库里是全角/直角引号、用户文档常被机器转成半角。
#: **一换一、不改变长度**，所以「归一后比对」的下标可以直接映射回原文下标。
#: 全仓只此一份表——`meaning_backfill_cli` 的切片比对也从这里取（见下）。
...
def _norm(s: str) -> str:
    """原文归一：去所有空白（全角/半角）+ 引号归一，用于按原文定位下标。

    引号那一档是血泪教训：文档写 `却道"天凉好个秋"！`、库里是 `却道“天凉好个秋”！`，
    只去空白就定位不到，该句的含义会被静默写成 `null`（学生丢一道题）。
    `meaning_backfill_cli.slice_by_doc_boundaries` 复用本函数做切片比对，
    **不要再各写一份表**，否则两个模块的判据会漂移。
    """
    return "".join((s or "").split()).translate(_QUOTE_FOLD)
