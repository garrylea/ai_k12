...
- **场景路由（勿随意切换）**：judgment/grading/structuring/hint 的 primary 是 `deepseek-v4-flash`（快模型，同题 ~19s 判对）；`qwen3.7-max` 是 reasoner（难几何题 >90s 仍超时），只作 fallback；tutoring 辅导主模型 `qwen3.7-max`；图片转录走独立 `transcribe` 场景（`qwen3-vl-plus`，fallback qwen-vl-max），辅导不因带图切换模型。
- **per-scene timeout 已接线**：各 capability 的 chat 调用传 `retry.yaml` 的 per-scene timeout（如 judgment 90s、explanation 60s），调超时改 yaml 即可。
