...
- 内容生产走 data-refinery **旁路** `dictation_cli`（见 `docs/data-refinery-使用手册.md` §4.9），**不接** convert→extract→publish→db_loader 四阶段主线，不产出 `cards`、不写 `questions`。只看 `convert_cli` 一步。
- **实施状态（2026-09-15）**：**设计已定，改造尚未实施**。现库仍是改造前形态——`dictation_passages`（挂 `question_id`）+ 50 行 `poem_dictation` 题 + `main_error_books(source='dictation')` 行。改造步骤见上条 spec §6；改造前 `docs/data-refinery-使用手册.md` §4.9 的表名请读作 `dictation_passages`。
...
- **语文默写判题与错因已解耦（2026-09-14 起）**：`POST /training/dictation/judge` **纯程序判对错、不等 LLM**（实测 12.6s → 25ms），答错回 `feedbackPending=true`；错因文案由 `POST /training/dictation/feedback` 单独取（实测 ~2s，关了 thinking）。错因端点只生成话术——服务端用纯函数 `evaluateDictation` 重算差异，**不写错题本**（重复调 `judgeDictation` 会二次写 `main_error_books`）。前端提交后立刻渲染对错 + 正文对比，错因区转圈等文案。**正文差异视图展示层带标点**（`diffChineseInOriginalText` 把标点回投原文；判对错口径仍忽略标点与空格）。
