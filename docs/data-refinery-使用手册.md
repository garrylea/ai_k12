...
**现成模板**：`inputs/interpretation/诗文重点字词.md`（39 个篇名从库里导出、已去重；
九上/九下重复收录的 9 篇合并成一段，不给册次时**一次写两册**）。格式说明见同目录 `README.md`。
...
| 册次 | 不给就按篇名匹配——重复收录的 9 篇会一次写入两册；只在「同一篇两册要写不同字词」时才需要显式写 `册次：下册` |
| 正文 | 以库里已校验的 `body` 为准，管线自己切句 |
| 逐句译文 | 没给就由本地模型生成（想覆盖时**只给译文**、按句序一行一条；**别给原文**——两册正文标点可能不同，给了会对不上） |

**三种格式任选**（Markdown 两种、JSON 一种）：

```markdown
# 岳阳楼记                    ← 写法 A：`#` 就是篇目（文件里没有 `##` 时）
...
〔北国〕指我国北方。          ← 课本注释原样：词由括号界定，无需分隔符

〔须〕等到。
```

```jsonl
{"work_title":"岳阳楼记","key_terms":[{"term":"谪守","gloss":"因罪贬谪流放，出任外官"}]}
```

行写法全部容错：行首编号可有可无（`1.` / `1、` / `①` / `-` / `*` / `（1）`）、
词可用 `〔〕`/`[]`/`【】` 界定（写法 B）、分隔符 `：`/`:`/`＝`/`=` 或首个空白都认
（`1 则:那么` → `term=则`，`gloss=那么`）；`<!-- … -->` 批注与 `---` 水平线整段跳过。

**注音保留**：`〔滕子京谪（zhé）守巴陵郡〕…` 里的拼音**原样入库**（学生要看得见读音），
只有「在正文里定位」与「前端高亮」用去注音形式（`滕子京谪守巴陵郡`，因正文里没有注音）。
因此 `/training/interpretation/start` 对每个词下发两个字段：`term`（带注音，展示用）
与 `plain`（去注音，前端高亮定位用）。

**篇名匹配已做归一**：全角/半角括号、`·` 两侧空格差异都能对上；只给词牌名（`水调歌头`）
也能匹配到库里的 `水调歌头(明月几时有)`。**extract 与 load 共用同一套**（曾因只有 extract 归一、
loader 精确匹配，导致 extract 通过的篇目 load 时被跳过）。

**译文是混合模式**：输入给了 `sentences`（Markdown 里用 `>` 两行一组：第一行原文第二行译文；
JSON 里 `sentences:[{text,translation}]`）就以输入的为准、**不调模型**；没给才由管线生成
（primary=local，fallback=deepseek-flash）。
