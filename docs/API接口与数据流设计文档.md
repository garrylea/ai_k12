...
| GET | `/api/training/dictation/passages` | 语文默写篇目清单（配置页用）。仅返回 `questions.is_active=1` 且 `dictation_passages.verified=1` 的**已校验**篇目，按 `sort_order, id` 排序。**只出篇名 + 册次**——作者/朝代/正文是学生要作答的三个判题字段，一律不下发（防答案泄露）。响应：`{passages: [{questionId, workTitle, semester}]}`。本清单不排除该生已标记「不再展示」的篇目。 | MVP |
