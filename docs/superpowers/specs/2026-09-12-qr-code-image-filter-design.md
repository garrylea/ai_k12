...
1. 试卷切题前识别出二维码图片，并把它从题目内容中剔除（独占一行则整行删除，
   与正文同行则只摘掉引用、保留正文）；
...
- 图片引用解析**复用 `image_scan._resolve_disk_path`**（已有三候选路径逻辑），
  避免第三份复制粘贴。
- 整行剔除镜像 `image_scan.scan_page` 的两遍式做法：先收集命中的 ref，再逐行处理。
  逐行判断用 `IMAGE_REF_RE` 匹配（比 `image_scan` 里 `f"![]({ref})"` 的写法更稳，
  兼容带 alt 的引用），末尾 `.strip()` 去掉首尾空行。
- **行内还有正文时只摘掉引用本身、保留正文**：二维码独占一行时整行删掉；
  若形如 `下列抽样方式：![](images/qr.jpg) 最合适的是`，删掉引用后保留该行文字。
  绝不因为删图而丢题干文字（`image_scan` 的整行删法在这里不适用，
  因为它过滤的是确定无正文的装饰性图标行）。
