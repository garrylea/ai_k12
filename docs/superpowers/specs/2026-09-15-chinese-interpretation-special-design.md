...
## 2. 核心原则：古诗文专项是独立子系统（**本次确立，已同步主文档**）
...
**为什么不进错题本**（2026-09-15 用户裁决，PRD §7.4 已写入例外）：错题本不是「错过的题的统一记事本」，而是**接主线清零门禁的待办队列**。它现有的三项机制对篇目级作答**都没有落点**：

| 错题本机制 | 对「题」（`questions`） | 对「篇目」（古诗文专项） |
|---|---|---|
| 重做—清零（`is_cleared`） | 题可重做，答对即清零 | 篇目当然可以重默，但「清零」清的是**该学科课程/单元/学期的解锁门禁**；专项不接主线，没有解锁可放行 |
| 错题级别递进（`level`） | 再错升级，随变式练习（§7.10）强化 | 无变式生成物 —— §7.10 的变式以**题目知识点**为轴，篇目没有知识点维度 |
| 错题练习数据源（§6.3） | 拉未清零错题逐题重做 | 拉进来只能变成「再默一遍」，与本专项自己的配置页（范围 + 题量 + 指定篇目）完全重复 |

佐证：「入本但没人读」不是假设，是改造前的实测状态 —— 库里有 `main_error_books(source='dictation')` 7 行，而 `ErrorPracticePage.tsx` 硬编码 `MATH_SUBJECT_ID`，这 7 行一条都读不到。它们既不进任何页面，也不参与任何门禁。
...
| 文档 | 更新内容 |
|---|---|
| `docs/K12智学系统-产品需求文档.md` | §6.3 训练轨：写入「古诗文专项是独立子系统」及其边界（试题类不适用）；**§7.4 错题本：来源表加「古诗文专项 → 不入本、不参与门禁」一行 + 例外说明**（该节是全系统错题本口径的唯一权威处，只改 §6.3 会让 PRD 自相矛盾） |
| `docs/K12智学系统-架构设计文档.md` | §4.2 核心子系统：新增「语文古诗文专项」子系统；§6.2 主要数据表：新增 `chinese_passages` |
| `docs/K12智学系统-数据库设计文档.md` | §1.1 通用规则：**允许 JSON 类型**（新增 JSON 行 + 驱动差异说明）；§3 表定义：新增 `chinese_passages`（**该表此前未收入本文档**，`dictation_passages` 当时也没写进来，故是新增而非替换）；§2 ER 总览同步 |
...
| `docs/data-refinery-使用手册.md` | §4.9 表名改 `chinese_passages`；「入库后怎么验」的孤儿检查 SQL 改业务键重复检查；幂等段去掉 `question_id`（改造未实施，故该节加了「改造待实施」提示） |
| `docs/ai-core-changelog.md` | 本次变更条目 |
| `docs/superpowers/specs/2026-09-13-chinese-dictation-special-design.md` | 顶部加「取代说明」横幅 + 在 §4.1 / §4.1.1 / §4.2 / §4.3 / §5 第 6 条 / §5 第 7 条 / §6 第 5 步 / §6 守卫段 / §8 / §9 逐处内联标注 |

> **执行状态（2026-09-15）**：上表**均已执行完毕**（另：数据库设计文档的 §7.1 MVP 清单与 §8 变更日志、架构文档 §4.2.5 错题本子系统两处也一并补了，属本清单的延伸）。
>
> **API 两份：只加变更标记，未改契约**。`docs/API接口与数据流设计文档.md` 与 `docs/api/openapi.yaml` 必须与**实际实现**一致（项目铁律：两份互为对照），而本次改造**尚未实施** —— 现在把 `questionId` 改成 `passageId` 会让文档描述不存在的接口。故两份都只在 `judge` 端点处加了一条「2026-09-15 设计变更，待实施」标注，**契约本身一字未动**，改造落地时随端点一起改。
>
> **范围外发现**：`docs/K12智学系统-后端Web服务设计文档.md` 不在本清单，但它整体已滞后于实现（repo 树只列到 `progress.repo.ts`、多个模块标「待建」而实际已建，且从未收录 dictation）。这是本次改造之前就存在的欠账，**不在本次范围内**，另行处理。
