...
### 3.6 选择题选项拆分（split_options，Python 确定性）

content 里找连续的 A,B,C,D 选项标记序列。标记两种体系（同一题不混用，括号式
优先）：

- **括号式** `(A)(B)(C)(D)`：全/半角括号均兼容
- **裸字母式**（2026-09-07 增）：行内 `'A. x B. y'`（点后须空格+内容，防
  `'A、B、C、D'` 枚举误切）或竖排 `'A'`/`'A.'` 单独成行

找到 4 连标记后按布局判别（标记是否跨行）：

- **行内布局**（4 标记同一行，如西城模拟二 题3「题干+选项一行+图片下一行」）：
  题干 = 第一个标记前的内容 **+ (D) 所在行行尾之后的尾部内容**（题干配图常排
  在选项后面，尾部图/文字整体归题干，不吞进 D；D 行尾的 `'[图]'` 占位符一并
  剥掉——MinerU 的图片位置标记）
- **跨行布局 + 内容在标记上方**（图选项标准排版：字母标在图/内容下方，MinerU
  线性化为 `'图[+说明]\nA\n图\nB\n...\nD'`；判别条件 = 4 个标记都独占行且
  D 行后无内容）：B/C/D 取上一标记行与本标记行之间的所有行；A 取其上方最近
  的图片行到 A 行之间的内容（更早的图归题干）。真实样本：东城 题1、西城
  题1（鼓图）、物理昌平 题2（图+说明文字）
- **跨行布局（默认）**（标记在上、内容/图在标记行之间，如 `'A.\n图\nB.\n图'`，
  2024 西城 题2 格式，D 后有内容）：选项 = 标记间内容，D 取到 content 末尾
  （自配图不被截走归题干）
- 找不到 4 连标记 → 非选择题或格式不符，返回 (原 content, None)，不拆
  （交 LLM 完整性兜底复核）
...
LLM 输入：单题 content + 已有知识点列表（见 §4.4）。content 的呈现形式
（2026-09-07 起）：选择题若 `split_options` 已确定性拆分成功，以
「题干：/选项A：/选项B：/选项C：/选项D：」分行形式呈现——完整性检测基于
拆分结果判（避免 LLM 对图选项的 content-above 排版误判「选项缺失」后用
regenerated 覆盖正确拆分）；拆分失败（非选择题/格式不符）时用原文。
...
### 4.2.1 标注必填字段校验 + 重试/备选/跳过（2026-09-07 增）

LLM 返回的标注 item 经 `_validate_item` 必填字段校验，任一缺陷触发重试
（此前静默吞异常留 `type=""`，publish 才被 pydantic 拦）：

- **type**：合法枚举（choice/fill_blank/true_false/short_answer/proof），缺失/非法 = 缺陷
- **difficulty**：1-5 整数，缺失/非法 = 缺陷（不 clamp 掩盖）
- **knowledge_points / suggested_new_kps**：必须为列表（可空）
- **completeness**：必须存在；`is_complete=false` 时必须有 `regenerated`
- **regenerated**（存在时）：content 非空；choice 必须 4 项 options 且
  label=={A,B,C,D}、text 非空；非 choice 不应带 options
- **一致性**：Python 已确定性拆出完整 4 选项时，LLM 判 `is_complete=false`
  或重写 options 与 Python 拆分不一致 = 缺陷（防 LLM 重写丢内容）

重试流程（`_label_one`）：主模型 → 主模型重试 → 备选模型（fallback_labeler）
各一次；全部失败返回 None，**跳过该题不写 JSONL**，打印 [WARN] 日志并写入
`labeling_issues.jsonl`（`type: "label_failed"`）供人工复核。多题批内某题
item 无效时降级为单题走同一重试流程。extract 结束汇总被跳过题号。

**答案与校验无关**（2026-09-07 澄清）：LLM 返回答案不构成缺陷、不触发重试。
原文无答案（Python 对齐 answer 为空）时，`_fill_from_item` 直接清空
`regenerated.answer`（无答案留空，做题时大模型补，见 §5）；其他字段照常
入库。prompt 同步约束：完整性检测不检查答案、修复不得补答案。
...
- `_is_pure_answer(text)`：有答案部分 且 **不含完整的选择题选项序列**（连续 A,B,C,D，复用 `question_splitter._find_option_marks`）

> **2026-09-12 修**：`_is_pure_answer` 原判据是「无选择题选项 (A)-(D) 无题干词（如图/下列）」——用「出现过任意一个 (A)」和关键词黑名单，在真实数据上必然误判：纯答案的解析正文里有概率记号 `\therefore P (A) = \frac{8}{12}`（命中括号判据）和「如图所示」「如图，连接 OC」（命中题干词判据），于是纯答案被当成「试卷+答案版」→ 走 case 4b 丢弃真试卷。改为结构判据「含连续 A–D 选项组」后，全库 4 个「试卷无答案 + 答案有答案」案例判定全部正确（海淀202507/朝阳202507/西城202507 判为纯答案走 4a；海淀202607 的答案 PDF 实为含答案版试卷，含 4 选项 → 判非纯答案走 4b，符合预期）。
>
> **2026-09-12 修**：`_derive_answer_path` 原只用 `md_path.with_name(...)`（只换文件名、父目录不变），而 `convert` 对每个 PDF 生成同名子目录（`X-试卷/X-试卷.md` 与 `X-答案/X-答案.md`），父目录不同 → 答案 md 永远配对不上 → 答案静默丢弃（全库 34 份试卷中 10 份「试卷无答案」的卷全中，`labeling_issues.jsonl` 里 76 条「答案缺失」即来自此）。现同目录找不到时按父目录名再找兄弟目录。
>
> **2026-09-12 修**：`_has_answer_section`（≥5 次「解：」）与 `split_page`（只认「参考答案/答案及评分/评分参考」关键字进入答案区）口径不一致。case 4a 合并的答案文件若没有这类标题（海淀202507 直接 `## 一、选择题` + 答案表），合并结果会被当成更多题干（题号重复、答案全空）。现在 case 4a 合并前会补一个规范化答案区标题 `# 参考答案`。
>
> **已知遗留（未修）**：`_has_answer_section` 只看末尾 500 字符，答案文件把「参考答案/答案及评分」写在标题（文件头部）时会漏判。全库实测 2 份答案 md（数学202507海淀模拟二、物理202407西城模拟二）命中，另有 15 份试卷 md 的全文关键字在尾部窗口外——若改成全文扫描需先确认这 15 份不会误判为「含答案」。
