...
| 13 | **「未定价模型」会被记成免费（有意不处理）** | `llm_models` 的两个价格列是 `NOT NULL DEFAULT 0`，而 `0` 在本设计中表示「真免费」（本地模型）。因此**管理员新建模型时漏填价格 → 该模型所有调用 `cost = 0`**，且因为报表里抓「算不出成本」用的是 `cost IS NULL`（`unpricedCalls`），这个漏填在报表上**看不出来**、只会让成本偏低。<br>**用户裁决（2026-09-19，两次独立评审提出后）：不在本期处理**——本期的核心指标是 **token 数（输入/输出分开）**，成本精度不是重点。故不加「价格必填」校验、不把价格列改为可空。缓解措施仅有一条：**Task 3 的 `seed-llm-prices` 必须给所有非本地模型回填真价**。<br>若将来成本准确性变重要，修法二选一：create 时价格必填（零类型连锁）；或价格列改可空并以 NULL 表示未定价（语义最正，但 `costPer1K` 类型要变可空，牵连 registry 与 `calculateCost`）。 |
