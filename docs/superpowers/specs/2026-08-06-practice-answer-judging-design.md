...
> **【2026-09-08 修订注记】** PRD §7.4 已由「双错题本」修订为「主线错题本（全系统唯一）」；本文其余判题设计不受影响。
...
### 5.0 card_splitter 同行题拆行（正则）

**当前问题**：`card_splitter._split_paragraphs` 只按 `\n\n` 拆段，同一段内 `(1) $5x^{2}-1=4x$ ; (2) $4x^{2}=81$` 不拆，多道题挤在一行入库。前端 `preprocessContent` step 2.5 在运行时用正则拆，但只覆盖 `;` 分隔案，无 `;` 案（如 `(1) x²=4 (2) y²=9`）拆不开。

**修法**：在 `_split_paragraphs` 后新增 `_split_inline_questions(paragraph)`，对每段按 `(N)` 边界拆成独立行——正则覆盖 `;` 和标点后的 `(N)`，防 `与(2)类似` 误拆。

```python
# (N) 前必须是句末标点或空格，排除正文续接如"与(2)类似"
...
正则拆不开的边缘案（无 `;`、LaTeX 挡拆分）由 LLM labeler 输出的 `questions[].text` 做 **content 重组**（兜底，见 §5.2）。
...
6. 前端-集成：**删除 `preprocessContent` step 2.5**（管线已保证每题独立成行）。`CourseDetailPage` 练习卡**结构化渲染**（intro + 可点题块，正则兜底）+ `practiceStore`（key by question.text）+ 答题列表 + 解析 + `api.ts`。
...
### 15.1 练习卡对错结果是否在卡片上持久显示

**现状（2026-08-09 发现）**：练习卡每题旁的 ✓/✗ 从内存 `practiceStore.answers` 读取，结果列表"完成"按钮调 `reset()` 清空 store，导致**关闭结果列表后卡片上的对错标记全部消失**；卡片不从后端重新拉取状态，重开卡片回到"全未做"。

**为何悬而未决**：涉及掌握式学习闭环与错题清除门槛的产品定位，权衡未定。

- 反对持久化（支持现状）：重新作答简单（每次开弹窗是新会话）、卡片保持干净、MVP 实现轻。
- 支持持久化：反馈丢失断学习闭环；与"主线错题清除才能解锁"门槛冲突（卡片层无从体现"哪些题还没掌握"）；错题已落库 `main_error_books` 但卡片未取用。

**关键依赖（决定方案前必须先定）**：
1. **数据模型**：目前只存错题，"做对/已掌握"未记录。要持久显示对/错，需先决定是否记录"做对"状态，否则卡片只能区分"错"与"其他（未做 or 做对）"。
2. **订正流程**：错题能否在卡片上重做？重做对了如何标记（移出错题本 / 标已订正）？直接关系错题清除门槛落地。
3. **徽标粒度**：✓/✗ 二态 还是 ✓/✗/已订正 三态。

**候选方向（头脑风暴，未选）**：
- A 持久化到卡片（后端拉取每题状态 + 订正）--最贴掌握式学习，工作量大。
- B 卡片保持干净，对错只在结果列表 + 错题本--最简单，缺即时反馈。
- C 混合：卡片轻量持久徽标，详细解析在结果列表/错题本，点错题可订正--平衡，中大工作量。
- D 会话内保留（"完成"不清空，reset 移到"重新练习"/换卡）--最小改动过渡方案，非持久。

**结论**：暂不实现，等错题清除门槛的产品设计明确后，再据此决定数据模型与方案。在此之前，已知一个独立 bug 需修：多 group 卡片 `answers[${gi}-${q.n}]` 与 `record` 写入的 `answers[q.n]` key 不一致，导致多 group 卡片 ✓/✗ 不显示（单 group 正常）。
