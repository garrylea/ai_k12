...
- **交互式终端（stdin 是 TTY）**：当场提示并读取输入：
  ...
  按 `(年级, 考试类型, 年份)` 分组缓存，同组只问一次（试卷与答案不会问两遍），
  本组剩余文件复用该答案。
...
| 表头原文 | mid | 命中规则 | 区县 |
| --- | --- | --- | --- |
| `海淀区2024-2025学年初三（上）期末考试卷和答案汇总` | `-2025学年` | 学年后为空 → 回退年份前缀 | `海淀区` |
| `2025-2026学年海淀区初二期末试卷&答案汇总` | `-2026学年海淀区` | 学年后 | `海淀区` |
| `2025-2026学年北京海淀区初一期末试卷&答案汇总` | `-2026学年北京海淀区` | 学年后 | `北京海淀区` |
| `2026海淀初三二模试卷&答案`（现有测试形态） | `海淀` | 无「学年」 | `海淀`（不变） |
...
- 询问流程：判不出时调用 `prompt_fn`；同组缓存只问一次；stub 断言 `上/下` 映射
- 区县解析：§4 四种表头形态
...
1. `python src/crawler_cli.py --site zgkao --url https://www.zgkao.com/shitiku/87761.html --subject 数学 --grade 初三`：
   产物落在 `data/数学/初中/first/<年>/`，文件名学期为 `(上)`、区县为 `海淀区`（不再是 `--2025学年`），
   且只下载初三（不再混入初一/初二）
