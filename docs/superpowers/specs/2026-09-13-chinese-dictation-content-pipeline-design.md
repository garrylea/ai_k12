...
2. **按页切掉注释块**：实测长文言文的**每一页是「上半页正文 + 下半页注释」**（醉翁亭记 page_060/061/062 都是如此），若直接按锚点取原始子串，**中间各页的注释块会落在区间内**——实测醉翁亭记切出 775 字且含 `〔〕` 与圈号，出师表/曹刿论战同样。故先对每页做「从第一行注释式内容（行首圈号 `①-⑳㉑+`，或含 `〔…〕`，兼作图注）起截断」，再按页序拼接。**实测该步对本来干净的篇目零影响**（岳阳楼记 446→446、湖心亭看雪 193→193），而醉翁亭记 775→584 且 `〔〕`/圈号全消；
