...
4. **规范化（顺序不可颠倒）**：先删行内注释角标 `$^{…}$`（实测 79/170 页含此类角标，是正文里指向注释的标记，不是正文），**再**去掉 OCR 插入的换行与空白；**保留全部标点与全角符号**（判题时的标点/空格忽略发生在 `normalizeChineseAnswer`，与存储无关——存储保留原文便于展示）。顺序必须先是「删角标」后是「收空白」：实测所有「汉字-空格-汉字」都由被删角标留下，先收空白会把角标粘进正文；
