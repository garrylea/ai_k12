...
### 4.1 `--export`：出模板（原文预填 + 已填含义回填）

导出范围 = **抽题池去掉「已有含义」这一条**：
`verified = 1 AND is_active = 1 AND JSON_LENGTH(sentences) > 0`。
即「把这些空填完就能进本专项」的全部篇目，不埋没任何一篇可填的。

首次导出（库里 `sentence_meanings` 全 `NULL`）出的就是「只有原文、两个空」的模板：
...
但**库里已填过的句子会连同含义/情感一起回填**——上面这个「两行全空」的样子只对未填过的篇目成立。
否则「导出 → 改一句 → 回写」这条修订路径会把该篇其余句子一并抹成 `null`
（模板里它们是空的，`--apply` 只认模板）。要清掉某句就把它那一行的值清空，那是显式表达。

其余行为：

- **同篇名多行合并成一段**：`chinese_passages` 按册收，九上/九下会重复收录同一首
  （实测 34 行只对应 25 首）。导出按篇名归一合并、正文相同的兄弟行不重复列；
  两行切句不同时（《出师表》一类文言文）两边原文都出现在模板里。
- **拒绝覆盖非空模板**：目标文件已存在且非空时报错退出，防手滑盖掉已填的稿子。
- **没有「整篇清回 `NULL`」的路径**：CLI 不提供这个操作，别假设有
  （整篇全空反而会被 §4.2 的守卫挡住不写库）。
...
- 篇目按 `work_title` 归一定位；重复收录（九上/九下）时**两册都写**，与解释管线一致。
- **整篇一句都没填 → 该篇不写库**：否则拿一份只剩原文的模板跑一次 `--apply` 就会把库里
  已填的含义整列抹成 `null`；同时也避免它带着 all-`null` 数组混进抽题池
  （抽题池只查 `sentence_meanings IS NOT NULL`，挡不住全 `null`）。空/半填的格子同样按「没填」处理。
