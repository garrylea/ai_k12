...
**现成模板**：`tools/data-refinery/inputs/interpretation/诗文重点字词.md` —— 39 个篇名从库里导出并去重（九上/九下重复收录的 9 篇合并成一段），用户只需在其下填字词。格式说明见同目录 `README.md`。

每篇只需要**篇名 + 重点字词（词 + 解释）**。**其余一律不用给**：

| 不用给 | 为什么 |
|---|---|
| 作者 / 朝代 | 库里已有（`chinese_passages.author` / `dynasty`） |
| 册次（`semester`） | 不给就按篇名匹配（`_find_rows(semester=None)`）——重复收录的 9 篇会**一次写入两册**，`sentenceIndex` 按各册正文分别计算。只在「同一篇两册要写不同字词」时才需要显式给 |
| 正文 | 以库里已校验的 `body` 为准，管线自己切句 |
| 逐句译文 | 没给就由模型生成（想覆盖时**只给译文**、按句序一行一条；**不要给原文**——实测《十五从军征》两册正文标点不同，给了原文必然与其中一册对不上，自检会拒收） |

字段名**中英文都认**（解析器做别名归一，可以混用）：

| 语义 | 认的写法 |
|---|---|
| 篇名 | `work_title` / `篇名` / `诗文名` / `题目` |
| 册次（可选） | `semester` / `册次` / `册`（值 `上册` / `下册`） |
...
一行一篇（JSONL），或一个文件一个 JSON 数组，两种都支持。`semester` 可省：

```jsonl
{"work_title":"岳阳楼记","key_terms":[{"term":"谪守","gloss":"因罪贬谪流放，出任外官"},{"term":"越明年","gloss":"到了第二年"}]}
{"work_title":"醉翁亭记","key_terms":[{"term":"环","gloss":"环绕"},{"term":"蔚然","gloss":"茂盛的样子"}]}
```

#### 格式 B：Markdown（手写用，推荐）

`#` / `##` 标题 = 篇名，一个文件可放多篇；其余非空行按「词 + 分隔符 + 解释」解析。
**篇名下不需要写 `册次`**（只在要单独指定某一册时才写）：
...
| 3 | 册次是否由你提供 | **不用给**（2026-09-16 简化的口径）——不给就按篇名匹配，九上/九下重复收录的 9 篇**一次写入两册**（`sentenceIndex` 按各册正文分别算，实测《出师表》上册 26 句 / 下册 27 句各归各的）。只在「同一篇两册要写不同字词」时才显式写 `册次：下册`。同理**作者/朝代也不用给**（库里已有） | §7.1 |
...
### 15.1 提交后的一处简化（2026-09-16，用户反馈）

用户看过模板后反馈「太麻烦了，作者不需要我提供……我只需要为每一篇提提字词」。据此：

1. **模板去掉作者/朝代批注与册次行**，从 48 段缩到 **39 段**（重复收录的 9 篇合并），
   只留 `# 篇名` + 空行；生成位置 `tools/data-refinery/inputs/interpretation/诗文重点字词.md`，
   同目录 `README.md` 说明格式。篇名仍从库里导出（库里格式不统一：有半角括号、`·` 前后空格不一），
   避免手抄错字。
2. **「不给册次」由防御路径升为主路径**：`_find_rows(semester=None)` 按篇名匹配、命中几行写几行，
   本来就是既有行为（原先只是兜底），现在明确推荐。冒烟实测《出师表》（上册 26 句 / 下册 27 句）
   两册各出一项、`sentenceIndex` 各按本册正文计算，无串扰。
3. **解析器补「整段跳过 HTML 注释」**（单行/跨行）：模板原先用 `<!-- 作者：… -->` 做提示，
   不跳过会被拆出「`<!-- 作者`」这种垃圾词、再报一堆「定位不到」把真问题淹掉。补 3 条测试。
