...
> **2026-09-11 更新（qwen3.8-max 换代后修正）**：本文件下面描述的「思考阶段标题」启发式与流式超时行为已被后续实现取代，以下为准（详见 `docs/ai-core-changelog.md` 2026-09-11 条）：
> - **标题改为按段落切块**：砍掉末尾 ```` ```json ```` 块后按 `\n` 切段，取最后一个「首句已完成」段落的 `标签：`（标签 ≤12 字）或首句/首分句（≤20 字）作为当前阶段标题；首句未写完的段落不产出标题、沿用上一段（避免流式逐字闪烁），段内稳定、换段才变。旧的「冒号短标题」规则基于 qwen3.7-max 的 CoT 格式，对 qwen3.8-max 的散文式思考链命中 0 条（真实数据实测），故折叠态一度退化成不断变长的整行。
> - **流式超时改为空闲超时**：`streamChat` 不再用从请求开始的墙钟硬超时（原 tutoring=45s，会把难题的长时间思考拦腰砍断），改为「收到数据即重置」的空闲超时，阈值取 `retry.yaml.streaming.firstTokenTimeoutMs`(3s) / `interTokenTimeoutMs`(10s)；空闲触发抛 `TimeoutError` → 前端「AI 响应超时」。
> - **出错/超时保留已流出的思考**：前端 `setLastAssistantError` 不再清空 content/reasoning，错误气泡叠在思考下方；后端 catch 在已有 reasoning/content 时 best-effort 落库部分 assistant（content 空填 `[生成中断]`）。
...
6. **`AuxChatPanel` / `ReasoningBlock`**
   ...
   - **折叠态（流式中）**：显示 reasoning 最后一行（`reasoning.split('\n').filter(Boolean).pop()`），实时滚动更新 = "thinking 阶段标题"效果；旁边可保留跳动三点。（> 2026-09-11：改为显示当前段落的合成标题，见文首更新）
