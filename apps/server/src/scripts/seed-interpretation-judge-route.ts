/**
 * 语文古诗文解释专项判题路由：`interpretation_judge` 场景 = 本地模型优先、deepseek-flash 兜底。
 * 读 YAML routes.interpretation_judge，幂等写进 llm_routes（已 seed 的库靠本脚本补路由；
 * seed-llm-config.ts 是 skip-if-exists，不会更新既有行）。
 * 镜像 seed-dictation-feedback-route.ts（llm_routes 存的是 model_key 字符串，不是模型 id）。
 *
 * 运行：cd apps/server && npx tsx src/scripts/seed-interpretation-judge-route.ts
...
const SCENE = 'interpretation_judge';

async function main() {
  const rule = routeConfig.routes.interpretation_judge?.find((r) => r.subject === '*')
    ?? routeConfig.routes.interpretation_judge?.[0];
  if (!rule) throw new Error('YAML 缺少 routes.interpretation_judge');
