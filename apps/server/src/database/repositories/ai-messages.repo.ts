...
export class AiMessagesRepository {
  ...
  async create(row: Omit<AiMessageRow, 'id' | 'created_at' | 'deleted_at'>): Promise<number> {
    const [result] = await this.pool.execute<ResultSetHeader>(
      `INSERT INTO ai_messages
       (dialogue_id, role, content, reasoning, type, attachments, model, token_input, token_output, response_time_ms, safety_flag)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [row.dialogue_id, row.role, row.content, row.reasoning, row.type, row.attachments, row.model, row.token_input, row.token_output, row.response_time_ms, row.safety_flag],
    );
  ...
  async createMany(rows: Array<Omit<AiMessageRow, 'id' | 'created_at' | 'deleted_at'>>): Promise<void> {
    ...
    const values = rows.flatMap((r) => [r.dialogue_id, r.role, r.content, r.reasoning, r.type, r.attachments, r.model, r.token_input, r.token_output, r.response_time_ms, r.safety_flag]);
