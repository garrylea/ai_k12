...
describe('practiceStore', () => {
  it('setSession 重置 answers/hints 并记录 session', () => {
    usePracticeStore.getState().record('0-1', 'x=2', { questionId: 1, isCorrect: true, method: 'exact', analysis: null });
  ...
  it('record 按复合题号写入 answer 且不影响其它题', () => {
    ...
    usePracticeStore.getState().record('0-2', 'x=3', { questionId: 2, isCorrect: false, method: 'ai', analysis: '粗心', errorType: 'calculation' });
  ...
  it('record 判定失败时带 failed 标记', () => {
    usePracticeStore.getState().setSession(42, QUESTIONS);
    usePracticeStore.getState().record('0-1', 'x=2', { questionId: null, isCorrect: false, method: 'ai', analysis: null }, { failed: true });
  ...
  it('loadResults 用 DB 结果填充 answers（key = questionN）', () => {
    ...
    usePracticeStore.getState().loadResults(42, QUESTIONS, [
      { questionN: '0-1', questionText: 'q1', studentAnswer: 'x=2', isCorrect: true, method: 'exact', analysis: null, errorType: null },
    ]);
  ...
  it('loadResults 同卡重入时保留在途作答（current 优先）', () => {
    ...
    usePracticeStore.getState().record('0-2', 'x=3', { questionId: 99, isCorrect: true, method: 'exact', analysis: null });
    usePracticeStore.getState().loadResults(42, QUESTIONS, [
      { questionN: '0-1', questionText: 'q1', studentAnswer: 'x=2', isCorrect: false, method: 'ai', analysis: '错', errorType: null },
    ]);
