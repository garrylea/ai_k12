      ...
      <div className="w-[92vw] max-w-2xl max-h-[80vh] overflow-auto bg-[var(--bg-card)] rounded-2xl shadow-xl p-6">
        ...
        <ul className="space-y-2">
          {questions.map(q => {
            ...
            return (
              <li key={q.text} className="flex items-center gap-3 p-3 rounded-lg border border-[var(--bg-subtle)]">
                ...
                {!correct && (
                  <button
                    onClick={() => setOpenText(openText === q.text ? null : q.text)}
