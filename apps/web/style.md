...
非学习阶段学生页背景（暖米白）
  Bg-Page     #F5F0E8   登录 / 选科 / 星图 / 个人中心 / 奖励册
...
辅线区分色
  Aux  #8B5A8E   [已废弃] 辅线已统一为品牌橘红 Brand-500 #FF6B35，与主线一致；
                 双轨通过文字标签区分（如入口选择页的"主轨学习"/"辅轨答疑"、Tag 文字"辅线"/"主线"），
...
**日夜间切换仅在学习沉浸阶段页面生效**。登录、注册、学科选择、章节星链图、个人中心、奖励册等非学习阶段学生页，以及家长端全部页面，均强制日间模式，物理屏蔽夜间切换。

| 类型 | 典型页面 | 主题容器 | 背景 | 风格关键词 |
|------|----------|----------|------|------------|
| 非学习阶段学生页 | P1.1 登录、P1.2 入口选择、P1.5 学科选择、P2.1 章节星链图、P5.1 个人中心、P5.2 奖励册 | 无，强制 `student-day` | `Bg-Page #F5F0E8` | 卡片式、分栏、强阴影、简洁大气、暖米白背景 |
| 学习沉浸阶段 | P2.2 课程详情、P2.3 AI 讨论、P2.4 课后作业、P2.5 判题、P2.6/P2.7 检测考试、P2.8 成绩报告、P3.x 辅线 | 有，启用日夜切换 | `Bg-Base #FAF6EE` / `#F5F5F7` | 护眼、沉浸、低刺激、简洁大气 |
...
| 错误反馈非纯红 | 用 #C44A3F 暗红 |
| 物理屏蔽 | 登录/注册/选科/星图导航/个人中心/奖励册/家长端强制日间 |
...
- **非学习阶段学生页**（登录、注册、学科选择、章节星链图、个人中心、奖励册等）：直接写死 `data-theme="student-day"`，**不使用** `.student-theme-container`，物理屏蔽夜间模式。背景使用 `Bg-Page #F5F0E8`。
...
- **PC App**：与 Web 使用**同一套布局**（≥1280px **不另做三栏**，2026-09-23 用户裁决，原「三栏」承诺已推翻）；非学习页卡片保持居中最大宽度，避免过宽。锁定中的剩余时间 pill 用 `--brand-*` 令牌（`--brand-100/500/600`），**不使用第二套配色**。
