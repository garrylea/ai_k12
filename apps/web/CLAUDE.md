...
在开始任何设计/编码任务前，必须先阅读以下基准文档：

| 文档 | 职责 | 必读章节 |
|---|---|---|
| **[K12智学系统-产品需求文档.md](../../docs/K12智学系统-产品需求文档.md)** | 产品需求基准（PRD） | §6 双轨学习、§7 核心功能、§8 输入交互、§13 MVP 范围 |
| **[UX-UI设计文档.md](../../docs/UX-UI设计文档.md)** | UI/UX 实现基准 | §2 设计系统、§5 页面清单、§7 响应式、§8 护眼与可访问性 |
| **[style.md](./style.md)** | **配色与风格唯一基准** | §2 配色、§3 字体、§4 间距/圆角/阴影、§7 星链图、§8 学科选择、§11 实施清单 |
| **[K12智学系统-架构设计文档.md](../../docs/K12智学系统-架构设计文档.md)** | 技术架构与数据模型 | §4 子系统划分、§5 数据流、§7 数据模型（前后端对齐时用） |
| **[API接口与数据流设计文档.md](../../docs/API接口与数据流设计文档.md)** | REST / WebSocket API 契约 | §4 端点清单、§5 数据流时序、`services/` 层对接时必读 |
...
5. **双轨视觉区隔**：主线与辅线同用 `Brand-500` 橘红（`#ff6b35`，见 style.md §2.1；辅线旧紫色 `#8B5A8E` 已废弃不用），双轨区隔靠 Tab 标题 / Tag 文字（「辅线」「主线」）与路由物理隔离（入口选择页，无跨轨链接），不靠颜色对比。
...
- **构建**：Vite 6 + TypeScript 5 + React 19
